document.addEventListener('DOMContentLoaded', () => {
    // ---- COMIENZA EL CÓDIGO EXTRAÍDO DE TU ARCHIVO ----
    const landingView = document.getElementById('landing-view');
    const appViewWrapper = document.getElementById('app-view-wrapper');
    const landingNavButtons = document.getElementById('landing-nav-buttons');
    const appNavButtons = document.getElementById('app-nav-buttons');
    const startNowBtnNav = document.getElementById('start-now-btn-nav');
    const ctaStartApp = document.getElementById('cta-start-app');
    ...
    function showApp() {
        ...
        appViewWrapper.classList.remove('hidden');
        landingNavButtons.style.display = 'none';
        appNavButtons.classList.remove('hidden');
        appNavButtons.style.display = 'flex';
        document.getElementById('language-selector-container').classList.remove('hidden');
        initApp();
    }

    function showLanding() {
        appViewWrapper.classList.add('hidden');
        landingView.style.display = 'block';
        appNavButtons.style.display = 'none';
        landingNavButtons.style.display = 'block';
        document.getElementById('language-selector-container').classList.remove('hidden');
        history.pushState("", document.title, window.location.pathname + window.location.search);
        updateTexts();
    }
    
    // Este código se ejecuta solo en app.html donde existen estos botones.
    // En otras páginas, estos elementos no existen y no se añaden los listeners.
    if(startNowBtnNav) startNowBtnNav.addEventListener('click', showApp);
    if(ctaStartApp) ctaStartApp.addEventListener('click', showApp);
    if(homeLogoBtn) homeLogoBtn.addEventListener('click', (e) => {
        e.preventDefault();
        // Si estamos en la app, nos lleva al landing. Si ya estamos en landing, no hace nada.
        if (!appViewWrapper.classList.contains('hidden')) {
            showLanding();
        } else {
            // Si el usuario está en la landing page principal (index.html), esto lo redirige.
    ...
    function navigateTo(viewId, pushToHistory = true) {
        ...
        appViewWrapper.querySelectorAll('.view').forEach(v => {
            v.style.position = v.id === viewId ? 'relative' : 'absolute';
    ...
    window.addEventListener('popstate', (event) => {
        if (event.state && event.state.view) {
            ...
            appViewWrapper.classList.contains('hidden') ? showLanding() : navigateTo('main-menu', false);
        }
    });
    ...
    function updateTexts() {
        ...
        document.querySelectorAll('[data-lang]').forEach(el => {
            ...
            if(langDict[key] && typeof langDict[key] === 'string' && el.tagName !== 'LABEL') {
                el.innerHTML = langDict[key];
            } else if (langDict[key] && el.tagName === 'LABEL') {
                 el.firstChild.textContent = langDict[key] + ' ';
            }
        });
        ...
        if (goalSelect) {
            ...
            for(const [value, text] of Object.entries(goalOptions)) {
                goalSelect.innerHTML += `<option value="${value}" data-lang="${value}">${text}</option>`;
            }
        }
        ...
        if(disciplineSelect){
            ...
            for(const [value, text] of Object.entries(disciplineOptions)) {
                disciplineSelect.innerHTML += `<option value="${value}" data-lang="${value}">${text}</option>`;
            }
        }
    ...
    function renderCalendar() {
        ...
        langDict.days_short.forEach(day => {
            const dayEl = document.createElement('div'); dayEl.textContent = day; dayEl.className = 'font-bold text-sm text-blue-300';
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            if (dayDate < new Date().setHours(0,0,0,0)) {
                ...
                dayEl.addEventListener('click', () => {
                    ...
                    selectedDate = dayDate; dayEl.classList.add('selected');
                });
            }
    ...
    function getStrengthWorkout(weekNum, weekPercent, langDict) {
        if (weekPercent < 0.6) { return weekNum % 2 === 1 ? langDict.strength_a : langDict.strength_b; } 
        else if (weekPercent < 0.85) { return weekNum % 2 === 1 ? langDict.strength_c : langDict.strength_d; } 
        else { return langDict.strength_maintenance; }
    }
    function getDetailedWorkout(weekNum, totalWeeks, dayIndex, goalType, level, langDict, paces) {
        const l = { beginner: 0, intermediate: 1, advanced: 2 }[level];
        const isTri = goalType.includes('tri'); const weekPercent = weekNum / totalWeeks;
        ...
        const workoutType = { 0: isTri ? 'easy_ride' : 'rest_day', 1: 'strength', 2: 'quality1', 3: 'easy_run', 4: 'quality2', 5: 'strength', 6: 'long_run_brick' }[dayIndex];
        switch(workoutType) {
            case 'strength': return getStrengthWorkout(weekNum, weekPercent, langDict);
            case 'quality1':
    ...
    function generateLongTermPlan(goalType, level, targetDate, timeGoal) {
        const langDict = translations[currentLang] || translations.es;
        const today = new Date(); const weeksUntil = Math.floor((targetDate - today) / (1000 * 60 * 60 * 24 * 7));
        const minWeeks = { 'race_10k': 8, 'half-marathon': 10, 'marathon': 16, 'tri-sprint': 8, 'tri-olympic': 12, 'tri-md': 16, 'tri-ld': 20 };
        if (weeksUntil < minWeeks[goalType]) { alert(langDict.error_min_weeks(minWeeks[goalType])); return null; }
        const paces = calculatePaces(goalType, timeGoal); let plan = [];
        ...
        for (let i = 0; i < weeksUntil; i++) {
            ...
            for (let day = 0; day < 7; day++) {
                ...
                if (phase === langDict.taper) {
                    ...
                    } else { weeklyWorkouts[dayName] = workout; }
                } else { weeklyWorkouts[dayName] = getDetailedWorkout(weekNum, weeksUntil, day, goalType, level, langDict, paces); }
            }
    ...
    function displayLongTermPlan(plan) {
        ...
        plan.forEach(week => {
            ...
            const orderedDays = [langDict.days_short[1], langDict.days_short[2], langDict.days_short[3], langDict.days_short[4], langDict.days_short[5], langDict.days_short[6], langDict.days_short[0]]; // Mon to Sun
            orderedDays.forEach(day => {
                const workout = week.workouts[day] || langDict.rest_day;
                html += `<li class="flex border-b border-blue-800/50 py-2"><strong class="w-12">${day}:</strong><span>${workout}</span></li>`;
            });
            html += `</ul></div>`;
        });
        html += '</div>';
    ...
    document.getElementById('goal-type')?.addEventListener('change', (e) => {
        ...
        if(timeGoalContainer) timeGoalContainer.classList.toggle('hidden', !['race_10k', 'half-marathon', 'marathon'].includes(e.target.value));
    });
    ...
    // Al cargar, decidimos qué mostrar
    const initialHash = window.location.hash.substring(1);
    if (initialHash.startsWith('main-menu') || initialHash.startsWith('long-term-planner') || initialHash.startsWith('today-planner') || initialHash.startsWith('plan-display')) {
        showApp();
    } else {
        showLanding();
    }
